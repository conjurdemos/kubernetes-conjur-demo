...
echo "Sidecar + Ruby API - $($cli exec -c test-app $sidecar_api_pod -- curl -s localhost)"
echo "Init Container + Ruby API - $($cli exec -c test-app $init_api_pod -- curl -s localhost)"
